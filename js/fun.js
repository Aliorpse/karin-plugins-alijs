import karin, { segment, redis, logger } from 'node-karin'
...
export const zanwo = karin.command(/^#?(全部)?(赞|超)我$/, async (e) => {
    ...
    let times = 0
    while(status == true){
        status = await e.bot.VoteUser(e.user_id)
        times += 1
        if(times == 1 && status == false){
            return e.reply(`${text}失败`,{ reply: true })
        }
    }
    return e.reply(`给你${text}好啦,一共${text}了${(times-1)*10}次`,{ reply: true })
})

export const jrys = karin.command(/^#?(今日运势|(J|j)rys)$/, async (e) => {

    //获取数据库
    let data = await redis.get(`Karin:fun_js:${e.user_id}_jrys`)
    if(data){
    ...
    await redis.set(`Karin:fun_js:${e.user_id}_jrys`, JSON.stringify(data))
