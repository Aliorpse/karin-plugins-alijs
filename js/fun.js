...
export const zanwo = karin.command(/^#?(全部)?(赞|超)我$/, async (e) => {
    ...
    //获取所有Bot实例
    let botList = []
    for(let key in karin.adapter){
        {
            let adapters = karin.getBotAll(true)
            forEach(adapters, (adapter) => {
                if(adapter.type != 'internal'){
                    botList.push(adapter.bot)
                }
            })
        }
    }
    ...
    for(let key in botList){
        let bot = botList[key]
        while(status == true){
            status = await bot.VoteUser(e.user_id)
            times += 1
        }
        status = true
    }
...
export const jrys = karin.command(/^#?(今日运势|(J|j)rys)$/, async (e) => {
    ...
    if(data){
        ...
        data = {
            fortune: fortuneList[Math.floor(Math.random() * fortuneList.length)],
            time: getDate()
        }
    }
    
    //处理数据
    if(getDate() != data.time){
        logger.info('Jrys >> 日期变更，重新抽取运势')
        data = {
            fortune: fortuneList[Math.floor(Math.random() * fortuneList.length)],
            time: getDate()
        }
    ...
    await level.set(`Karin:fun_js:${e.user_id}_jrys`, JSON.stringify(data))

    const current = data.fortune
    return e.reply([
        segment.at(e.user_id),
        ` 的${numToChinese(getDate())}号运势为……\n${current.fortuneSummary} | ${current.luckyStar}\n---\n${current.signText}\n>> ${current.unsignText}`
    ],{ recallMsg: 60 })
...
export const jrlp = karin.command(/^#?今日老(婆|公)$/, async (e) => {
    ...
    //获取数据库
    let data = await level.get(`Karin:fun_js:${e.user_id}_jrlp`)
    if(data){
        data = JSON.parse(data)
    }else{
    ...
    await level.set(`Karin:fun_js:${e.user_id}_jrlp`, JSON.stringify(data))
